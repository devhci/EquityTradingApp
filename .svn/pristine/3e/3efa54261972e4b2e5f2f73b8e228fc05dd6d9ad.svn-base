...
@Action(value = "forgot-password", results = {
		...
		@Result(location = "forgotPassword-view", type = "tiles", name = "failure"),
...
public class ForgotPasswordAction extends ActionSupport {
	...
	public String execute() {
		...
		String	result = forgotpasswordservice.authenticate(username,
				securityAnswer);
		if (result.equalsIgnoreCase(StringLiterals.FAILURE)) 
			addActionError("username and security answer didn't match");
		return result;
	}
	@Override
